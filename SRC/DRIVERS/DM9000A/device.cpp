    ...
    PU8 ptrBuffer)
{
    ...
    *(PU8)(ptrBuffer + 0) = (U8)m_szConfigures[CID_MAC_ADDR_0];
    *(PU8)(ptrBuffer + 1) = (U8)m_szConfigures[CID_MAC_ADDR_1];
    *(PU8)(ptrBuffer + 2) = (U8)m_szConfigures[CID_MAC_ADDR_2];
    *(PU8)(ptrBuffer + 3) = (U8)m_szConfigures[CID_MAC_ADDR_3];
    *(PU8)(ptrBuffer + 4) = (U8)m_szConfigures[CID_MAC_ADDR_4];
    *(PU8)(ptrBuffer + 5) = (U8)m_szConfigures[CID_MAC_ADDR_5];
    
    switch (m_szConfigures[CID_MAC_LOAD_MODE])
    {
        case 0x00 :
            RETAILMSG(TRUE, (TEXT("[DM9ISA]: load NIC MAC address form register file \r\n")));
            break;
            
        case 0x01 :
            RETAILMSG(TRUE, (TEXT("[DM9ISA]: load NIC MAC address form boot PAR register setting \r\n")));
            
            for (i = 0; i < 6; i++)
                *(PU8)(ptrBuffer + i) = (U8)DeviceReadPort(0x10 + i);
                
            break;
            
        case 0x02 :
            RETAILMSG(TRUE, (TEXT("[DM9ISA]: load NIC MAC address form AUTO check Flash Data setting \r\n")));
            
            /*
            PU8 flash_mac_save_address;
            if(((U8)m_szConfigures[CID_MAC_ADDR_0] == *flash_mac_save_address[0]) &&
               ((U8)m_szConfigures[CID_MAC_ADDR_1] == *flash_mac_save_address[1]) &&
               ((U8)m_szConfigures[CID_MAC_ADDR_2] == *flash_mac_save_address[2]))
            {
             RETAILMSG(TRUE,(TEXT("[DM9ISA]: AUTO check ok \r\n")));
             *(PU8)(ptrBuffer + 3) = *flash_mac_save_address[3];
             *(PU8)(ptrBuffer + 4) = *flash_mac_save_address[4];
             *(PU8)(ptrBuffer + 5) = *flash_mac_save_address[5];
            }
            else
            {
             RETAILMSG(TRUE,(TEXT("[DM9ISA]: AUTO check fail , use register file default \r\n")));
            }
            */
            break;
            
        case 0x03 :
            RETAILMSG(TRUE, (TEXT("[DM9ISA]: load NIC MAC address form LOAD EEPROM \r\n")));
            
            pcurr = (PU16) & m_szEeprom[m_szEepromFormat[EID_MAC_ADDRESS]];
        ...
        case 0x04 :
            RETAILMSG(TRUE, (TEXT("[DM9ISA]: load NIC MAC address form Random create \r\n")));
            ...
            {
                for(i = 0; i < 3; i++)
                    save_Random_mac[i] = (U8)(Random() & 0xff);
            }
            
            *(PU8)(ptrBuffer + 3) = save_Random_mac[0];
            *(PU8)(ptrBuffer + 4) = save_Random_mac[1];
...
void NIC_DEVICE_OBJECT::DeviceRegisterAdapter(void)
{
    ...
    if (m_szConfigures[CID_INTERMEDIATE])
        attr |= NDIS_ATTRIBUTE_INTERMEDIATE_DRIVER
    ...
    NDIS_HANDLE hConfig)
{
    ...
    RETAILMSG(TRUE, (TEXT("[DM9ISA]: read register IoAddress %x !\r\n"), m_szConfigures[CID_IO_BASE_ADDRESS]));
    RETAILMSG(TRUE, (TEXT("[DM9ISA]: read register IrqNumber %x !\r\n"), m_szConfigures[CID_IRQ_NUMBER]));
    RETAILMSG(TRUE, (TEXT("[DM9ISA]: read register Int_Active_High %x !\r\n"), m_szConfigures[CID_INT_ACTIVE]));
    RETAILMSG(TRUE, (TEXT("[DM9ISA]: read register Flow_Control_Enable %x !\r\n"), m_szConfigures[CID_FC_ENABLE]));
    RETAILMSG(TRUE, (TEXT("[DM9ISA]: read register MAC_Load_Mode %x !\r\n"), m_szConfigures[CID_MAC_LOAD_MODE]));
    RETAILMSG(TRUE, (TEXT("[DM9ISA]: read register MAC_ADDR_0 %x !\r\n"), m_szConfigures[CID_MAC_ADDR_0]));
    RETAILMSG(TRUE, (TEXT("[DM9ISA]: read register MAC_ADDR_1 %x !\r\n"), m_szConfigures[CID_MAC_ADDR_1]));
    RETAILMSG(TRUE, (TEXT("[DM9ISA]: read register MAC_ADDR_1 %x !\r\n"), m_szConfigures[CID_MAC_ADDR_2]));
    RETAILMSG(TRUE, (TEXT("[DM9ISA]: read register MAC_ADDR_2 %x !\r\n"), m_szConfigures[CID_MAC_ADDR_3]));
    RETAILMSG(TRUE, (TEXT("[DM9ISA]: read register MAC_ADDR_3 %x !\r\n"), m_szConfigures[CID_MAC_ADDR_4]));
    RETAILMSG(TRUE, (TEXT("[DM9ISA]: read register MAC_ADDR_5 %x !\r\n"), m_szConfigures[CID_MAC_ADDR_5]));
...
// m_szCurrentSettings[SID_MEDIA_CONNECTION_STATUS] = NdisMediaStateConnected;
    ...
    m_szCurrentSettings[SID_GEN_TRANSMIT_BUFFER_SPACE] = ETH_MAX_FRAME_SIZE;
    m_szCurrentSettings[SID_GEN_RECEIVE_BUFFER_SPACE] = ETH_MAX_FRAME_SIZE * 32;
...
{
    if (!m_szConfigures[CID_NEED_IO_SPACE])
        return;
    ...
    DEBUG_PRINT((TEXT("[DM9ISA_d]: Tries to map io space with %X\r\n"), phyAddr.LowPart));
    
    if (!(uBase = (U32)MmMapIoSpace(phyAddr, 0x10000, FALSE)))
        THROW((ERR_STRING("Fails to map io space")));
        
    DEBUG_PRINT((TEXT("[DM9ISA_d]: The mapped address is %X\r\n"), uBase));
...
{
    if (!m_szConfigures[CID_NEED_INTERRUPT])
        return;
...
{
    if(0x03 != m_szConfigures[CID_MAC_LOAD_MODE]) return;
    ...
    U32 uRetries)
{
    ...
    {
        if ((DeviceReadPort(uPort) & uMask) == uExpected)
            break;
            
        for (delay_cnt = (uInterval / 50); delay_cnt; delay_cnt--)
        {
            NdisStallExecution(50);
        }
        
        NdisStallExecution(uInterval % 50);
    } // of retry loop
...
void NIC_DEVICE_OBJECT::SetConnectionStatus(bool bConnected)
{
    if (bConnected)
    {
        ...
        {
            DEBUG_PRINT((TEXT("[DM9ISA_d]: Send connection!\r\n")));
            m_szCurrentSettings[SID_MEDIA_CONNECTION_STATUS] = NdisMediaStateConnected;
            NdisMIndicateStatus(m_pUpper->GetNdisHandle(), 
    ...
    else
    {
        ...
        {
            DEBUG_PRINT((TEXT("[DM9ISA_d]: Send disconnection!\r\n")));
            m_szCurrentSettings[SID_MEDIA_CONNECTION_STATUS] = NdisMediaStateDisconnected;
            NdisMIndicateStatus(m_pUpper->GetNdisHandle(),
...
BOOL NIC_DEVICE_OBJECT::DeviceCheckForHang(void)
{
    ...
    {
    	DEBUG_PRINT((TEXT("[DM9ISA]: DeviceCheckForHang cr %x  lastread %x  !\r\n"), cr, lastread));
        return TRUE;
    }
        
    // 2. tx idle while tqueue out of resource
    if (m_pUpper->DriverIsOutOfResource() && (DeviceHardwareStatus() & NIC_HW_TX_IDLE))
    {
    	DEBUG_PRINT((TEXT("[DM9ISA]: tx idle while tqueue out of resource  !\r\n")));
        return TRUE;
    }
