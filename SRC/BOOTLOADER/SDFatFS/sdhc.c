...
#define Input16(addr)		(*(volatile UINT16 *)(addr))
...
void SDHC_ErrorInterruptHandler(SDHC* sCh) {
	...
	status = sCh->m_sErrorStatus = Input16(sCh->m_uBaseAddr+SDHC_ERROR_INT_STAT);

	if( status & SDHC_ADMA2_ERROR ) {
		UART_Printf("SDHC_ADMA2_ERROR[%x]\n", Input32(sCh->m_uBaseAddr+SDHC_ADMA_ERROR));
...
void SDHC_InterruptHandler(SDHC * sCh) {
	...
	sCh->m_sNormalStatus |= status  = Input16(sCh->m_uBaseAddr+SDHC_NORMAL_INT_STAT);
...
//		SDHC_NORMAL_INT_CLEAR(sCh, 4 );
	...
	if( status & SDHC_DMA_SIG_INT_EN ) {
		...
		SDHC_SetSystemAddressReg(sCh, Input32(sCh->m_uBaseAddr+SDHC_SYS_ADDR) );
	}
...
void SDHC_SetDriveStrength(SDHC*sCh, u8 cStrength ) {
	...
	if(sCh->m_eChannel == SDHC_CHANNEL_0 || sCh->m_eChannel == SDHC_CHANNEL_1 ) {
		...
		Outp32( sCh->m_uBaseAddr+SDHC_CONTROL4, 0x3<<16);				// Clock
	}
...
void SDHC_DisplayCardInformation(SDHC * sCh)
{
	...
	sCh->m_sReadBlockLen = (u16)((Input32( sCh->m_uBaseAddr+SDHC_RSP2 )>>8) & 0xf);
	sCh->m_sReadBlockPartial = (u16)((Input32( sCh->m_uBaseAddr+SDHC_RSP2 )>>7) & 0x1);
	sCh->m_sCSize = (u16)(((Input32( sCh->m_uBaseAddr+SDHC_RSP2 ) & 0x3) << 10) | ((Input32( sCh->m_uBaseAddr+SDHC_RSP1 ) >> 22) & 0x3ff));
	sCh->m_sCSizeMult = (u16)((Input32( sCh->m_uBaseAddr+SDHC_RSP1 )>>7)&0x7);
